...
export const StateContextProvider = ({ children }) => {
            ...
            params: [
              {
                ...
                rpcUrls: [
                  "https://eth-sepolia.g.alchemy.com/v2/a6DIWovRu8rerU9atZKY19XbJHrgJm5y",
                ],
                nativeCurrency: {
                  name: "ETH",
                  symbol: "ETH",
  ...
  const createNewList = async ({
    ...
    if (contract && signer) {
      const newListing = await contract.newListing(
        title,
        description,
        target,
        image,
  ...
  const buyListing = async (_contract, _arbiter) => {
    if (_arbiter && _contract) {
      try {
        const eth = await currentProduct[7].convertUSDToEther(
          currentProduct[0]
        );
